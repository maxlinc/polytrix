...
class Pacto::Extensions::Loaders::URIMapLoader < Pacto::Extensions::Loaders::YamlOrJsonLoader
  ...
  private
  def self.build_simple_contract service_definition, group_name, service_name, server, file
    service_signature = "#{service_definition['method'].upcase} #{service_definition['uriTemplate']}"
    puts "Registering '#{service_signature}' as '#{group_name} - #{service_name}' on #{server}"
    # FIXME: What about scheme?
    host = "https://#{server}:443"
  ...
  def self.convert_template path
    path.gsub(/{(\w+)}/, ':\1') if path
  end
end

# Pacto.configuration.strict_matchers = false
# contracts = Pacto::Extensions::Loaders::URIMapLoader.load('pacto/uri_map.yaml')
